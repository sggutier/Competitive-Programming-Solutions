...
void testCase(int n) {
    ...
    // for(int i=0; i<tot; i++) {
    //     // printf("%c %d => %d\n", lin[i]<limS? '#' : lin[i]-limS+'a', owner(i), SA[i]);
    //     for(char *c=lin + SA[i]; *c; c++)
    //         printf("%c", *c<limS? '#' : *c-limS+'a');
    //     printf(" \t\t %d => %d\n", owner(SA[i]), LCP[i]);
    // }
    // printf("\n");
    ...
    for(int i=0, j=0, tcnt=0; j<tot; j++) {
        ...
        //     printf("%d ", w);
        // printf("\n");
        // for(char *c=lin + SA[j]; *c; c++)
        //     printf("%c", *c<limS? '#' : *c-limS+'a');
        // printf("======== %d => %d\n", owner(SA[j]), LCP[j]);
        if(tcnt > n) {
            while(i<j && tcnt > n + 2) {
                if(--tips[owner(SA[i])] == 0)
                    tcnt-=2;
